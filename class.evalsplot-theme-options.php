...
class evalsplot_Theme_Options {
	...
	function __construct() {
        ...
        add_action( 'admin_enqueue_scripts', 'evalsplot_enqueue_options_scripts' );
		
		add_action( 'admin_menu', array( &$this, 'add_pages' ) );
		add_action( 'admin_init', array( &$this, 'register_settings' ) );
		
		if ( ! get_option( 'evalsplot_options' ) )
			$this->initialize_settings();
	}

	/* Add page(s) to the admin menu */
	public function add_pages() {
		$admin_page = add_theme_page( 'evalsplot Options', 'evalsplot Options', 'manage_options', 'evalsplot-options', array( &$this, 'display_page' ) );
		
		// documents page, but don't add to menu		
		$docs_page = add_theme_page( 'evalsplot Documentation', '', 'manage_options', 'evalsplot-docs', array( &$this, 'display_docs' ) );
			}

	/* HTML to display the theme options page */
	public function display_page() {
		echo '<div class="wrap">
		<h2>evalsplot Options</h2>';
		...
		echo '<form action="options.php" method="post" enctype="multipart/form-data">';
			...
			settings_fields( 'evalsplot_options' );
			
			echo  '<h2 class="nav-tab-wrapper"><a class="nav-tab nav-tab-active" href="?page=evalsplot-options">Settings</a>
	<a class="nav-tab" href="?page=evalsplot-docs">Documentation</a></h2>';
	...
	public function display_docs() {	
		// This displays on the "Documentation" tab. 
	 	...
	 	echo '<div class="wrap">
		<h1>evalsplot Documentation</h1>
		<h2 class="nav-tab-wrapper">
		<a class="nav-tab" href="?page=evalsplot-options">Settings</a>
		<a class="nav-tab nav-tab-active" href="?page=evalsplot-docs">Documentation</a></h2>';
		
		include( get_stylesheet_directory() . '/includes/evalsplot-theme-options-docs.php');
	...
	public function get_settings() {
		...
		'title' 	=> '' ,// Not used for headings.
		'desc'   => 'Author Account', 
		'std'    =>  evalsplot_author_user_check( 'sharer' ),
		'type'    => 'heading'
		);	
	...
	public function display_general() {
		...
		echo '<p>These settings manaage the behavior and appearance of your evalsplot site. There are quite a few of them!</p>';		
	}
	...
	public function display_setting( $args = array() ) {
		...
		$options = get_option( 'evalsplot_options' );
		...
		switch ( $type ) {
			...
			case 'checkbox':
				...
				echo '<input class="checkbox' . $field_class . '" type="checkbox" id="' . $id . '" name="evalsplot_options[' . $id . ']" value="1" ' . checked( $options[$id], 1, false ) . ' /> <label for="' . $id . '">' . $desc . '</label>';

				break;

			case 'select':
				echo '<select class="select' . $field_class . '" name="evalsplot_options[' . $id . ']">';
			...
			case 'radio':
				$i = 0;
				foreach ( $choices as $value => $label ) {
					echo '<input class="radio' . $field_class . '" type="radio" name="evalsplot_options[' . $id . ']" id="' . $id . $i . '" value="' . esc_attr( $value ) . '" ' . checked( $options[$id], $value, false ) . '> <label for="' . $id . $i . '">' . $label . '</label>';
			...
			case 'textarea':
				echo '<textarea class="' . $field_class . '" id="' . $id . '" name="evalsplot_options[' . $id . ']" placeholder="' . $std . '" rows="5" cols="60">' . format_for_editor( $options[$id] ) . '</textarea>';
			...
			case 'medialoader':
				...
				echo '<input type="hidden" name="evalsplot_options[' . $id . ']" id="' . $id . '" value="' . $options[$id]  . '" />
  <br /><input type="button" class="upload_image_button button-primary" name="_evalsplot_button' . $id .'" id="_evalsplot_button' . $id .'" data-options_id="' . $id  . '" data-uploader_title="Set Default Header Image" data-uploader_button_text="Select Image" value="Set/Change Image" />
</div><!-- uploader -->';
			...
			case 'password':
				echo '<input class="regular-text' . $field_class . '" type="text" id="' . $id . '" name="evalsplot_options[' . $id . ']" value="' . esc_attr( $options[$id] ) . '" /> <input type="button" id="showHide" value="Show" /> ';
			...
			case 'text':
			default:
				echo '<input class="regular-text' . $field_class . '" type="text" id="' . $id . '" name="evalsplot_options[' . $id . ']" placeholder="' . $std . '" value="' . esc_attr( $options[$id] ) . '" />';
	...
	public function initialize_settings() {
		...
		update_option( 'evalsplot_options', $default_settings );
	...
	public function register_settings() {
		...
		register_setting( 'evalsplot_options', 'evalsplot_options', array ( &$this, 'validate_settings' ) );


		foreach ( $this->sections as $slug => $title ) {
			add_settings_section( $slug, $title, array( &$this, $this->section_callbacks[$slug] ), 'evalsplot-options' );
		}		
	...
	public function create_setting( $args = array() ) {
		...
		add_settings_field( $id, $title, array( $this, 'display_setting' ), 'evalsplot-options', $section, $field_args );
...
$theme_options = new evalsplot_Theme_Options();

function evalsplot_option( $option ) {
	$options = get_option( 'evalsplot_options' );
